...
class CompanyApp(tk.Tk):
    ...
    def apply_filters(self, name_filter, city_filter, business_type_filter, phone_number_filter, contact_status_filter):
        ...
        if city_filter:
            filtered_data = [company for company in filtered_data if city_filter.lower() in company["City"].lower()]
        if business_type_filter:
            filtered_data = [company for company in filtered_data if business_type_filter in company.get("Business Type", "")]
        ...
        if contact_status_filter:
            filtered_data = [company for company in filtered_data if contact_status_filter.lower().strip() == company.get("Contact Status", "Not contacted").lower().strip()]
    ...
    def update_treeview(self):
        ...
        for company in self.filtered_data:
            self.tree.insert("", "end", iid=company["Name"], text=company["Name"], values=(
                ...
                "Yes" if company.get("Has Website", "").lower() == "yes" else "No",
    ...
    def save_company_data(self):
        ...
        if filename:
            fieldnames = self.company_data[0].keys() if self.company_data else []
            try:
